...
export const getCurrentLocation = async (): Promise<GeolocationData> => {
  try {
    ...
    const response = await axios.get<IpApiResponse>('http://ip-api.com/json/?fields=status,country,countryCode,region,regionName,city,zip,lat,lon,timezone,isp,org,as,query');
