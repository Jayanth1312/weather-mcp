  ...
  if (fs.existsSync(envPath)) {
    ...
    for (const line of lines) {
      ...
      if (trimmed && !trimmed.startsWith('#') && trimmed.includes('=')) {
        const [key, ...valueParts] = trimmed.split('=').filter(part => part !== undefined) as [string, ...string[]];
    ...
    tools: [
      {
        ...
        description:
          "Get current weather information for a city or coordinates",
        inputSchema: {
          type: "object",
          properties: {
            city: {
      ...
      },
      {
        ...
        description:
          "Get current weather information for your current location (automatically detected based on IP address)",
        inputSchema: {
          type: "object",
          properties: {},
          required: [],
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (name === "get_current_weather") {
    try {
      ...
      if (args?.["city"]) {
        params.city = args["city"] as string;
      }

      if (args?.["lat"] !== undefined && args?.["lon"] !== undefined) {
        params.lat = args["lat"] as number;
        params.lon = args["lon"] as number;
      }
      ...
      throw new McpError(
        ...
        `Failed to get weather data: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
  ...
  if (name === "get_current_location_weather") {
    try {
        ...
        content: [
          {
            type: "text",
            text:
              `📍 Detected Location: ${location.city}, ${location.country}\n\n` +
              `Current weather:\n` +
              `Temperature: ${weatherData.current.temperature}°C\n` +
              `Feels like: ${weatherData.current.feelsLike}°C\n` +
              `Humidity: ${weatherData.current.humidity}%\n` +
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to get location or weather data: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
  if (request.params.uri === "weather://current") {
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to read weather resource: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
  ...
  if (request.params.uri === "weather://current-location") {
    try {
      ...
      throw new McpError(
        ErrorCode.InternalError,
        `Failed to read location weather resource: ${error instanceof Error ? error.message : "Unknown error"}`
      );
    }
  }
